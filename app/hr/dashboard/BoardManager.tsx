...
// ─── CONFIG ────────────────────────────────────────────────────────────────────
const API = "/strapi/api/leadership-members";
...
const photoUrl = (p: string) => {
  if (!p) return "https://ui-avatars.com/api/?name=Director&background=07518a&color=fff";
    ...
    >
      ...
      <div className="absolute inset-0 p-6 flex flex-col justify-end">
        <div className="translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
          ...
          <p className="text-blue-200/90 text-xs font-semibold uppercase tracking-widest line-clamp-1 mb-4">{member.designation || "Director"}</p>
...
export default function BoardManager() {
  ...
  const fetchMembers = useCallback(async () => {
    try {
      const { data } = await axios.get(`${API}?populate=photo&sort=order:asc&pagination[pageSize]=100`, authHeader());
      const mapped = (data.data || []).map((m: any, i: number) => {
        const a = m.attributes || m;
        const photoData = a.photo;
        ...
        return {
          ...
          bio: a.bio || "",
          linkedin: a.linkedin || "",
          photo: firstPhoto?.url || firstPhoto?.attributes?.url || "",
          order: a.order ?? i,
        };
  ...
  // ── Sync Logic ──
  const performSync = useCallback(async (reordered: BoardMember[]) => {
    ...
    try {
      ...
      message.success({ content: "Board Directory Order Synchronized", key: "sync" });
    } catch (err) {
    ...
    try {
      ...
      const payload = {
        data: {
          ...
          ...(imageId ? { photo: imageId } : (fileList.length === 0 ? { photo: null } : {})),
        }
      };
  ...
  if (loading) return (
    <div className="fixed inset-0 flex flex-col items-center justify-center bg-white z-[9999]">
      ...
      <h2 className="text-sm font-black uppercase tracking-[0.2em] text-slate-400">Initializing Board Directory</h2>
    </div>
  );
      ...
      <nav className="sticky top-0 z-[1000] bg-white/85 backdrop-blur-2xl border-b border-slate-200 px-6 py-4">
        <div className="max-w-[1800px] mx-auto flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              <h1 className="text-xl font-black tracking-tight text-slate-900">Board Directory</h1>
      ...
      <main className="max-w-[1800px] mx-auto px-6 py-10">
        ...
        <div className="mb-10 bg-blue-600 rounded-[2.5rem] p-10 relative overflow-hidden shadow-2xl shadow-blue-200 flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <div className="relative z-10 max-w-xl">
            <span className="inline-block px-4 py-1.5 rounded-full bg-white/20 backdrop-blur-md text-white text-[10px] font-bold uppercase tracking-[0.2em] mb-4">Board of Directors Management</span>
            <h2 className="text-4xl font-black text-white mb-4 leading-tight">Master Board <br /> Structure & Order</h2>
            <p className="text-blue-100/80 font-medium leading-relaxed">Organize, customize, and drag-and-drop to reorder Brihaspathi Technologies' leadership team directory for dynamic rendering on the public website.</p>
          </div>
          <div className="relative z-10 flex gap-4">
            <div className="bg-white/10 backdrop-blur-md border border-white/20 p-6 rounded-3xl text-center w-32">
              ...
              <div className="text-[10px] font-bold text-blue-100 uppercase tracking-widest">Directors</div>
            </div>
            <div className="bg-white/10 backdrop-blur-md border border-white/20 p-6 rounded-3xl text-center w-32">
              <div className="text-3xl font-black text-white mb-1">{members.filter(m => m.photo).length}</div>
              <div className="text-[10px] font-bold text-blue-100 uppercase tracking-widest">With Photos</div>
            </div>
          </div>
        </div>
                ...
                {filteredMembers.map((m) => (
                  ...
                  >
                    <SortableCard
                      ...
                      onDelete={() => {
                        Modal.confirm({
                          title: 'Remove Director Profile?',
                          content: `Are you sure you want to delete ${m.name}? This will permanently remove them from the board directory.`,
                          okText: 'Delete',
                          okType: 'danger',
                          cancelText: 'Cancel',
                          className: 'pro-modal',
                          onOk: async () => {
                            try {
                              ...
                              message.success("Director Profile Removed");
                              fetchMembers();
                            } catch (err) {
          ...
          <DragOverlay adjustScale={false} dropAnimation={{ sideEffects: defaultDropAnimationSideEffects({ styles: { active: { opacity: "0.5" } } }) }}>
            {activeId ? (
              <div className="h-[420px] w-full">
                <DirectorCard member={members.find(m => m.id === activeId)} dragMode={false} isOverlay />
              </div>
            ) : null}
          </DragOverlay>
        </DndContext>
        ...
        {!loading && filteredMembers.length === 0 && (
          <div className="py-32 flex flex-col items-center justify-center bg-white rounded-[3rem] border border-slate-200 border-dashed">
            ...
            <p className="text-slate-500 mb-8 max-w-sm text-center">We couldn't find any director registry entries matching your current search terms.</p>
      ...
      >
        <div className="p-10">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="text-2xl font-black text-slate-900">{editing ? 'Modify Director' : 'Register Director'}</h2>
              <p className="text-sm font-medium text-slate-400 mt-1">Configure professional board member identity details.</p>
            </div>
          ...
          <Form form={form} layout="vertical" onFinish={handleSave} className="space-y-6">
            <div className="grid grid-cols-2 gap-6">
              <Form.Item label={<span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Legal Name</span>} name="name" rules={[{ required: true, message: 'Required' }]}>
                <Input placeholder="e.g. John Doe" className="h-12 rounded-2xl bg-slate-50 border-slate-200 focus:ring-blue-500 focus:border-blue-500" />
              </Form.Item>
              <Form.Item label={<span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Designation</span>} name="designation">
                <Input placeholder="e.g. Chairman" className="h-12 rounded-2xl bg-slate-50 border-slate-200" />
              </Form.Item>
            </div>
            ...
            <div className="flex gap-4 pt-4">
              ...
              <button type="submit" disabled={syncing} className="flex-[2] h-14 rounded-2xl bg-blue-600 text-white font-bold shadow-xl shadow-blue-600/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2">
                ...
                {editing ? 'Update Profile' : 'Confirm Board Registration'}
              </button>
            </div>
          </Form>
        </div>
      ...
      <AnimatePresence>
        {syncing && (
          ...
          >
            ...
            <span className="text-[11px] font-black uppercase tracking-[0.2em]">Synchronizing Board Directory</span>
          </motion.div>
        )}
      </AnimatePresence>
