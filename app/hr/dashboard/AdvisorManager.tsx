...
const photoUrl = (p: string) => {
  if (!p) return "https://ui-avatars.com/api/?name=Advisor&background=07518a&color=fff";
  ...
  return (
    <div
      ...
      className={`
        ...
        ${isOverlay ? "shadow-2xl ring-4 ring-blue-500/20 scale-[1.02] z-50" : "hover:shadow-2xl hover:-translate-y-1"}
      `}
    >
      {/* Photo Container */}
      ...
      <div className="absolute inset-0 p-6 flex flex-col justify-end">
        <div className="translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
          <div className="flex items-center gap-2 mb-2">
            <span className="px-2.5 py-0.5 rounded-full bg-blue-500/20 backdrop-blur-md border border-white/20 text-[10px] font-bold text-white uppercase tracking-wider">
              #{member.order + 1}
            </span>
          </div>
          <h3 className="text-xl font-bold text-white mb-1 line-clamp-1">{member.name || "Untitled Advisor"}</h3>
          <p className="text-blue-200/90 text-xs font-semibold uppercase tracking-widest line-clamp-1 mb-4">Board Advisor</p>
...
function SortableCard(props: any) {
  ...
  const style = {
    ...
    transition: isDragging ? "none" : transform ? transition : undefined,
...
// ─── MAIN COMPONENT ────────────────────────────────────────────────────────────
export default function AdvisorManager() {
  ...
  const [editing, setEditing] = useState<AdvisorMember | null>(null);
  ...
  const membersRef = useRef<AdvisorMember[]>([]);
  ...
  const fetchMembers = useCallback(async () => {
    try {
      const { data } = await axios.get(`${API}?populate=profile&sort=order:asc&pagination[pageSize]=100`, authHeader());
      const mapped = (data.data || []).map((m: any, i: number) => {
        ...
        return {
          ...
          name: a.name || "",
          bio: a.bio || "",
          linkedin: a.linkdin_link || a.linkedin || "",
          photo: firstProfile?.url || firstProfile?.attributes?.url || "",
          order: a.order ?? i,
        };
  ...
  // ── Sync Logic ──
  const performSync = useCallback(async (reordered: AdvisorMember[]) => {
    ...
    try {
      ...
      message.success({ content: "Advisor Directory Order Synchronized", key: "sync" });
    } catch (err) {
    ...
    try {
      ...
      const payload = {
        data: {
          name: values.name,
          bio: values.bio,
          linkdin_link: values.linkedin,
          order: editing ? editing.order : (members.length > 0 ? Math.max(...members.map(m => m.order)) + 1 : 0),
          ...(imageId ? { profile: imageId } : (fileList.length === 0 ? { profile: null } : {})),
        }
      };
  ...
  if (loading) return (
    <div className="fixed inset-0 flex flex-col items-center justify-center bg-white z-[9999]">
      ...
      <h2 className="text-sm font-black uppercase tracking-[0.2em] text-slate-400">Initializing Advisor Registry</h2>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-blue-100 selection:text-blue-900">
      ...
      {/* ── Top Navigation Bar ── */}
      <nav className="sticky top-0 z-[1000] bg-white/85 backdrop-blur-2xl border-b border-slate-200 px-6 py-4">
        <div className="max-w-[1800px] mx-auto flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              <h1 className="text-xl font-black tracking-tight text-slate-900">Advisors Management</h1>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="relative group flex-1 md:flex-none md:w-64">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-slate-100 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-blue-500/20 transition-all outline-none"
                value={searchTerm}
            ...
            <div className="flex items-center gap-2 bg-slate-100 p-1.5 rounded-2xl border border-slate-200">
              <button
                ...
                className={`px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${dragMode ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:bg-white hover:text-slate-900'}`}
              >
                Reorder Mode
              </button>
            </div>
      ...
      <main className="max-w-[1800px] mx-auto px-6 py-10">
        ...
        {/* Banner Section */}
        <div className="mb-10 bg-blue-600 rounded-[2.5rem] p-10 relative overflow-hidden shadow-2xl shadow-blue-200 flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <div className="relative z-10 max-w-xl">
            <span className="inline-block px-4 py-1.5 rounded-full bg-white/20 backdrop-blur-md text-white text-[10px] font-bold uppercase tracking-[0.2em] mb-4">HR Administration</span>
            <h2 className="text-4xl font-black text-white mb-4 leading-tight">Master Advisor <br /> Structure & Order</h2>
            <p className="text-blue-100/80 font-medium leading-relaxed">Organize, customize, and drag-and-drop to reorder Brihaspathi Technologies' Advisor registry for dynamic rendering on the public site.</p>
          </div>
          <div className="relative z-10 flex gap-4">
            <div className="bg-white/10 backdrop-blur-md border border-white/20 p-6 rounded-3xl text-center w-32">
              ...
              <div className="text-[10px] font-bold text-blue-100 uppercase tracking-widest">Advisors</div>
            </div>
            <div className="bg-white/10 backdrop-blur-md border border-white/20 p-6 rounded-3xl text-center w-32">
              <div className="text-3xl font-black text-white mb-1">{members.filter(m => m.photo).length}</div>
              <div className="text-[10px] font-bold text-blue-100 uppercase tracking-widest">With Portraits</div>
            </div>
          </div>
        </div>
                ...
                {filteredMembers.map((m) => (
                  ...
                  >
                    <SortableCard
                      ...
                      onEdit={() => {
                        setEditing(m);
                        form.setFieldsValue({ name: m.name, bio: m.bio, linkedin: m.linkedin });
                        setFileList(m.photo ? [{ uid: "-1", name: "profile", status: "done", url: photoUrl(m.photo) }] : []);
                        setModalOpen(true);
                      }}
                      onDelete={() => {
                        Modal.confirm({
                          title: 'Remove Advisor Profile?',
                          content: `Are you sure you want to delete ${m.name}? This will permanently remove them from the advisor directory.`,
                          okText: 'Delete',
                          okType: 'danger',
                          cancelText: 'Cancel',
                          className: 'pro-modal',
                          onOk: async () => {
                            try {
                              ...
                              message.success("Advisor Profile Removed");
                              fetchMembers();
                            } catch (err) {
          ...
          <DragOverlay adjustScale={false} dropAnimation={{ sideEffects: defaultDropAnimationSideEffects({ styles: { active: { opacity: "0.5" } } }) }}>
            {activeId ? (
              <div className="h-[420px] w-full">
                <AdvisorCard member={members.find(m => m.id === activeId)} dragMode={false} isOverlay />
              </div>
            ) : null}
          </DragOverlay>
        </DndContext>
        ...
        {!loading && filteredMembers.length === 0 && (
          <div className="py-32 flex flex-col items-center justify-center bg-white rounded-[3rem] border border-slate-200 border-dashed">
            ...
            <p className="text-slate-500 mb-8 max-w-sm text-center">We couldn't find any advisor registry entries matching your current search terms.</p>
            <button onClick={() => setSearchTerm("")} className="px-6 py-2.5 bg-blue-600 text-white rounded-2xl text-sm font-bold">Clear Search</button>
          </div>
        )}
      </main>
      ...
      >
        <div className="p-10">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="text-2xl font-black text-slate-900">{editing ? 'Modify Advisor' : 'Register Advisor'}</h2>
              <p className="text-sm font-medium text-slate-400 mt-1">Configure professional advisor credentials and identity details.</p>
            </div>
          ...
          <Form form={form} layout="vertical" onFinish={handleSave} className="space-y-6">
            <Form.Item label={<span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Legal Name</span>} name="name" rules={[{ required: true, message: 'Required' }]}>
              <Input placeholder="e.g. Dr. John Doe" className="h-12 rounded-2xl bg-slate-50 border-slate-200 focus:ring-blue-500 focus:border-blue-500" />
            </Form.Item>
            ...
            <Form.Item label={<span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Biography / Credentials</span>} name="bio">
              <Input.TextArea rows={4} className="rounded-2xl bg-slate-50 border-slate-200 resize-none" maxLength={1500} showCount />
            </Form.Item>

            <Form.Item label={<span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Portrait Image</span>}>
              <Upload
            ...
            <div className="flex gap-4 pt-4">
              ...
              <button type="submit" disabled={syncing} className="flex-[2] h-14 rounded-2xl bg-blue-600 text-white font-bold shadow-xl shadow-blue-600/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2">
                {syncing ? <Loader2 className="animate-spin" size={20} /> : <Save size={20} />}
                {editing ? 'Update Profile' : 'Confirm Advisor Registration'}
              </button>
            </div>
          </Form>
        </div>
      ...
      <AnimatePresence>
        {syncing && (
          ...
          >
            ...
            <span className="text-[11px] font-black uppercase tracking-[0.2em]">Synchronizing Advisor Registry</span>
          </motion.div>
        )}
      </AnimatePresence>
