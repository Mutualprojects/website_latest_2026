...
/* ================= FALLBACK DATA (DESENDING: HIGHEST YEAR FIRST) ================= */
...
export default function CircularDialJourney() {
  ...
  // Ensure rawData is ordered highest year first (e.g. 2026 -> 2006)
  const sortedRawData = useMemo(() => {
    if (!rawData.length) return FALLBACK_DATA;
    // Check if first entry year is smaller than last entry year, if so reverse it
    const copy = [...rawData];
  ...
  // Group raw entries by unique Year (Highest year first)
  ...
  // Default initial selection to 0 (which is HIGHEST YEAR 2026)
  useEffect(() => {
  ...
  // Rotation degrees for circular wheel
    ...
    <div className="w-full bg-white text-slate-900 py-8 px-4 sm:px-6 lg:px-8 font-sans min-h-[80vh] flex flex-col justify-center">
      <div className="w-full max-w-7xl mx-auto">
        ...
        <div className="text-center max-w-3xl mx-auto mb-6">
          ...
          {/* VIEW TOGGLE SWITCHER (CIRCULAR RADAR vs TIMELINE STREAM) */}
        ...
        {!loading && yearGroups.length > 0 && (
          <AnimatePresence mode="wait">
            {/* ================= OPTION 1: CIRCULAR RADAR VIEW (HIGHEST YEAR FIRST) ================= */}
            {viewMode === "circular" ? (
              <motion.div
                key="circular"
              ...
              >
                {/* LEFT COLUMN: CIRCULAR DIAL WHEEL */}
                <div className="lg:col-span-5 flex flex-col items-center justify-center py-4">
                  
                  {/* Big Outer Round Circle (Brand #07518a Background) */}
                  <div className="relative w-[300px] sm:w-[360px] aspect-square rounded-full bg-[#07518a] flex items-center justify-center shadow-2xl shadow-[#07518a]/30 border-4 border-[#07518a]/20">
                    ...
                    {/* Small Center Inner Round Circle (White Background with Brand Text) */}
                ...
                {/* RIGHT COLUMN: PROJECTS LIST FOR SELECTED YEAR (80VH SCROLLABLE) */}
              ...
              /* ================= OPTION 2: ALTERNATING TIMELINE STREAM VIEW (HIGHEST YEAR FIRST: 2026 -> 2006) ================= */
              <motion.div
                key="timeline"
                initial={{ opacity: 0, y: 15 }}
                animate={{ opacity: 1, y: 0 }}
                      ...
                      >
                        ...
                        <div className="w-full md:w-1/2 pl-14 md:pl-0 md:px-8">
                          ...
                          >
                            ...
                            {/* Body Image (If present) + Description */}
                            <div className="flex flex-col sm:flex-row gap-4 items-start">
                              {validImg && (
